  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const response = await fetch(`https://university-manager-scholaria-6.onrender.com/fetch_student_for_director`, {
  ...
  const fetchMessagesForConversation = async () => {
    ...
    try {
      ...
      const displayMessages: Message[] = conversationMessages.map((msg: BackendMessage) => {
        const msgSenderId = Number(msg.sender_id)
        const sender = msgSenderId === currentUserIdNum ? 'director' : 'student'
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  {student.role && (
                    <p className="text-xs text-purple-600 font-medium">{student.role}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-4 border-b border-gray-300 flex items-center gap-3">
              ...
              <div>
                ...
                {selectedStudent.role && (
                  <p className="text-xs text-purple-600 font-medium">{selectedStudent.role}</p>
                )}
              </div>
            </div>
              ...
              {messagesLoading && messages.length === 0 ? (
                ...
                messages.map((message) => (
                  <div
                    key={message.id}
                    className={`flex ${
                      message.sender === 'director' ? 'justify-end' : 'justify-start'
                    }`}
                  >
                    <div className={`flex items-start gap-2 ${
                      message.sender === 'director' ? 'flex-row-reverse' : 'flex-row'
                    }`}>
                      <div
                        ...
                        {editingMessageId === message.id ? (
                          <div className="space-y-2">
                            <input
                              ...
                              className="w-full px-2 py-1 text-gray-900 border border-gray-300 rounded focus:outline-none focus:border-purple-500"
                              autoFocus
                            />
                          ...
                          <>
                            ...
                            <p
                              className={`text-xs mt-1 ${
                                message.sender === 'director' ? 'text-purple-100' : 'text-gray-500'
                              }`}
                            >
                      ...
                      {message.sender === 'director' && editingMessageId !== message.id && (
            ...
            <div className="bg-white p-4 border-t border-gray-300">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-purple-500 text-white rounded-full hover:bg-purple-600 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
